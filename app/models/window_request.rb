...
class WindowRequest < ApplicationRecord
   ...
   def send_request_mail(window_request, mail_address)
      ...
      `echo '#{body_content}' | mail -s "$(echo -e "Redhawk check-in request #{window_request.version} (#{window_request.username}, #{window_request.bugid})\nContent-Type: text/html")" #{mail_cc_address} redhawk_qa@ansys.com#{rd_mail}` 
   end 
end
